...
export function ymanHeaders(json = true): Record<string, string> {
  const key = ymanApiKey();
  if (!key) {
    throw new ProviderHttpError(500, "missing_api_key", "缺少 YMAN_API_KEY");
  }
 ...
 */
export async function ymanPost(
  pathSuffix: string,
  body: unknown,
...
export async function ymanGet(pathSuffix: string): Promise<Record<string, unknown>> {
  const res = await fetchUpstream(`${ymanBase()}${pathSuffix}`, {
    method: "GET",
    headers: ymanHeaders(false),
  });
  return readYmanBody(res);
}

async function readYmanBody(res: Response): Promise<Record<string, unknown>> {
  const parsed = (await res.json().catch(() => ({}))) as unknown;
  const data =
    parsed && typeof parsed === "object" && !Array.isArray(parsed)
      ? (parsed as Record<string, unknown>)
      : {};
  if (!res.ok) throw ymanError(res.status, data);
  return data;
}

/**
 * YMan 的 HTTP 状态 → 本项目的错误信封。映射出来的 code 直接决定 runner 的行为：
...
export function ymanError(status: number, body: Record<string, unknown>): ProviderHttpError {
  ...
  if (mapped) {
    return new ProviderHttpError(status, mapped.code, upstreamMessage ?? mapped.message);
  }
  // 5xx 原样透传（runner 的可重试集合按 status >= 500 也能认出来）。
  return new ProviderHttpError(
    status,
    upstreamCode ?? `upstream_http_${status}`,
    upstreamMessage ?? `YMan 上游 HTTP ${status}`,
  );
}
...
const MAPPED_STATUS: Record<number, { code: string; message: string }> = {
  ...
  401: { code: "unauthorized", message: "YMan API Key 无效" },
