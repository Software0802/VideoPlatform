...
- **配额行**（`.composer__quota`）：位于输入卡工具栏芯片旁，文案「今日剩余 n/N」，`title` 悬浮显示完整重置提示（「今日已用 x/N，北京时间 0 点重置」）；剩余为 0 时 `data-empty="true"` 变暗，发送按钮与配额行下方各出现一条 `.composer__error--quota` 提示，禁用提交。数据来自 `GET /api/me`，缺失（请求失败）时整行不渲染，不假装数字。
...
- **输入卡** `.composer`：宽 `min(708px,100%)`，min-height 143，radius 26，padding `24px 22px 12px`，gap 46（工作室 22）。textarea 2 行（工作室 3–7 行随内容）。工具栏：三条路径芯片 + 「8s ⌄」（视频；4 / 6 / 8 / 10 循环，开启 harness 时追加 30 / 45 / 60）+ 「16:9 ⌄」（16:9 / 9:16 / 1:1 循环）；右侧簇：模型名 + 回形针（首帧，选中后白色）+ 30px 发送圆钮（生成中变 `conic-gradient` 进度环 + 22px 深色圆显示百分比）。
...
- 保留现有能力并按同一语言落位：生成中「取消」、失败态「重新生成 / 重做失败分镜」、空提示词的 `role="alert"` 错误行、成片与作品元信息里的成本（`≈ $x.xx`，账目不全时 `≥`）、模型名后的「· 模拟」标记、开启 harness 时时长循环追加 30 / 45 / 60 且模型名后带预估。
