...
- **订阅页兑换礼品码与积分流水**：「我的方案」卡新增礼品码输入框 + 兑换按钮（`POST /api/me/redeem`，成功后刷新顶栏积分并 toast 到账金额，404/409/429 分别显示对应中文错误）；「积分使用详情」「账单记录」两个链接改为打开一个抽屉，分页读 `GET /api/me/ledger`（后者带 `kind=grant` 只看充值/兑换），列表按时间倒序展示 `kind` 中文标签、金额、余额快照与备注。四档订阅卡在本节记录时（2026-09-06 夜）仍是占位（按钮 toast「即将上线」），**2026-09-07 凌晨起已换成真实购买路径**，见上「五个视图」第 5 点。
...
- 订阅四档价格曾是原型的美元占位值（`docs/plan-ui-genius-app.md` 明确按钮为「即将上线」toast，非真实购买路径）——**2026-09-07 凌晨起已改为真实人民币价格与真实购买路径，此条不再适用**，见上「五个视图」第 5 点与 `docs/design.md` §2i。
