...
- **输入卡** `.composer`：宽 `min(708px,100%)`，min-height 143，radius 26，padding `24px 22px 12px`，gap 46（工作室 22）。textarea 2 行（工作室 3–7 行随内容）。工具栏：三条路径芯片 + 「8s ⌄」（视频；时长档位**由服务端按当前 provider 能力下发**，2026-09-06 晚起，见 `docs/design.md` §2e/§6——默认 4 / 6 / 8 / 10 循环，开启 harness 时追加 30 / 45 / 60，某些上游模型是不同档位如 5/10）+ 「16:9 ⌄」（画幅同样由服务端下发，`VIDEO_PROVIDER_ORDER` 里所有有 key 的 provider 支持画幅的并集，通常仍是 16:9 / 9:16 / 1:1 循环）+「有声 / 无声」芯片（视频路径，2026-09-06 阶段一新增，见上；`audioAvailable=false` 时锁死无声，YMan 恒为不可控故如此）；右侧簇：模型名 + 回形针（首帧，选中后白色）+ 30px 发送圆钮（生成中变 `conic-gradient` 进度环 + 22px 深色圆显示百分比）。
