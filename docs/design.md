...
- **能力推导**：`modes` = 配了默认模型的视频 mode + 有 image 通道时的 `text_to_image`；画幅 / 时长档 / 分辨率 / 参考图上限全部从目录（`catalog.models`）推导，积分计价 `credits = 分辨率价 + 时长价`（上游 `/models` 直接给 `credits` 时优先采用：对象形状进档表、裸数字当单次平价）、`creditsPerCny` 折人民币再折 USD。
- **动态目录（N3.3）**：`catalog.source:"models-endpoint"` 时目录 = `/models` 快照（`data/relay-catalog/<id>.json`，含上游给的 `kind`/`supported_ratios`/`supported_resolutions`/`alias`/`canonical_id`/`credits`）∪ 配置覆盖；启动 30s 后首拉，之后每 `RELAY_CATALOG_REFRESH_MS`（默认 30 分钟、下限 60 秒）刷新，失败保留上次快照；discover 接口可手动刷。每次刷新与上一快照做 diff：新增模型记 info，消失模型记 warn 并发 `upstream_model_missing` 告警（`reason:"catalog"`，按 `relay:model` 去重）；**默认模型**（`video.defaults`）消失时该 mode 立即从 `capabilities().modes` 收缩（`RelayView.unavailableModes` 调用时读快照），路由自动跳到下一家，模型回来时自动恢复。
