...
- `supportsImageReference`（`capabilities()` 可选字段,省略=false）表示该生图 provider 的 `text_to_image` 请求接受 `referenceImages`、会改走图生图接口。openai 通道由 `OPENAI_IMAGE_EDITS_ENABLED=1` 打开,yman 生图通道由 `YMAN_IMAGE_EDITS_ENABLED=1` 打开,默认都关;ccgoai 已实测透传（生产已开）,YMan 是否透传未验证。开关关着时带参考图的请求在 `validate` 被 400 `invalid_argument` 拒绝。
...
- 错误语义两条例外（2026-09-13,`545580f`）:**结构化 5xx 是确定拒单**——`normalizeImageUpstreamError` 对 `status>=500` 且响应体为合法 OpenAI 错误信封（`error.message` 为字符串且 `code`/`type` 有其一）的应答给 `ProviderHttpError` 打 `upstreamRejected`,runner 的 `isAmbiguousSubmitError` 对它返回 false,按普通失败处理、可重试,不进 `uncertain_submit`;裸 5xx 与非结构化体仍走 §3 的模糊提交路径。**404 模型缺失告警**——创建 POST 收到 404 时除照常抛错外发 `upstream_model_missing` 告警（`notifyAlert`,`{provider,model,base}`,去重键 `provider:model`）;`agent/llm.ts` 的对话调用与 yman 的创建 POST 同规则（GET 轮询的 404 是任务句柄丢失,不告警）。

## 2c. 视频 provider 路由(可灵,2026-09-06,as-built)

方案 `docs/plan-kling-video.md`。可灵 provider(`klingProvider`,`src/lib/providers/kling/`)只声明 `text_to_video`/`image_to_video` 两种模式,是否被选中由 `VIDEO_PROVIDER_ORDER` 的次序决定——它要在次序表里、配了 `KLING_API_KEY`、未被判定耗尽才会被 `pickVideoProvider` 选中。其余模式由「声明支持它们的 provider」承接:`reference_to_video` 当前由 yman / grok 声明,`edit_video`/`extend_video` 只有 grok 声明(ORDER 内没有可用 provider 承接时提交返 503 `no_provider_available`),30/45/60 长片走 harness 管线(按 i2v+t2v 能力走 `VIDEO_PROVIDER_ORDER`,可灵 / YMan 都能接,生产 `HARNESS_ENABLED=true` 已开放)。已作为 `bcad123` 提交并于 2026-09-06 部署到生产(`api-singapore` 域名),详见 `docs/handoff.md` §0c。
...
模型目录 `catalog.ts`:模型 ID 必须用上游 `GET /v1/models` 的**展示名**(如 `minimax-h3`、`minimax-h3-933-图文`),旧展示名与内部名(如 `minimax-H3 文字`、`minimax_h3_t2v`)作别名识别;`YMAN_MODEL_CATALOG`(JSON)可逐字段合并追加/覆盖模型的档位与积分价目。默认 t2v 模型 `minimax-h3`(纯文生,不收参考图;价目 2026-09-13 沿用旧档、未经账单核实),i2v 模型 `minimax-h3-933-图文`(收参考图 ≤9 张)。
...
### 服务治理：健康冷却与确定拒单换家（2026-09-13 N3.4，as-built）

`src/lib/providers/health.ts`（`exhaustion.ts` 是它的兼容薄壳）按 **provider × 通道**（视频/图片分开）记健康：内存滑动窗口（最近 20 次 / 10 分钟的 `[ts, ok, ms]` 样本）+ 落盘冷却表 `data/provider-health.json`（原子写、重启延续）。冷却分级：`quota_exhausted` → `PROVIDER_EXHAUSTED_TTL_MS`（默认 6h）；`rate_limited` → 上游 `Retry-After`（`ProviderHttpError.retryAfterMs`，封顶 15 分钟），否则 60 秒起连续命中翻倍、成功一次归零；连续 3 次 `5xx / 连接失败 / 读超时` → 5 分钟冷却 + `relay_unhealthy` 告警（`provider:kind` 去重）。冷却到期进**半开**：只放一个探路任务（5 分钟认领 TTL），成功清零恢复、失败翻倍再冷却（上限 15 分钟）。路由（`pickVideoProvider`/`pickImageProvider`/fallback）、产品目录 `isProductAvailable` 都看 `isAvailable`；`/api/health` 的 `providerHealth` 与 `GET /api/admin/relays` 的 `health` 字段输出当前状态。

**确定拒绝换家**（`jobs/runner.ts` `switchProvider`）：提交被上游**确定拒绝**时——`isCertainRejection`（`providers/rejection.ts`）：4xx 业务拒绝（参数类 `invalid_argument`/`unsupported_mode`/`insufficient_balance` 除外，换家也一样错）、结构化拒单信封 `upstreamRejected`、`upstream_unavailable` 且 `phase:"connect"`（ECONNREFUSED/ENOTFOUND，连接没建起来请求不可能送达，`fetchUpstream` 只给这两种 errno 打 connect 标）——先 `recordOutcome` 记账冷却，再把任务改交给 `currentProviderId(..., {exclude})` 选出的下一家。排除集 = 本任务**已试过的全部家**（`providerSwitches` 的 from/to 并集 + 当前家），换家上限 `RELAY_MAX_SWITCHES`（默认 2，按任务计）。换家重算 `model` / 上游档位 / 分辨率 / 音轨，产品标签按新家模型反查（对不上就摘掉），`priceCny` **只降不升**，`costUsdEstimate` 重算（长片用 `harnessSubmitEstimateUsd`，30/45/60 目标总长不归一）；每次换家落 `job.providerSwitches[]`（from/to/code/at）。用户**点名产品**的任务（`productPicked`）不换家——换成别家是交付他没选的产品——确定拒绝直接 `product_unavailable` 失败退款（「所选模型暂时不可用，请换一个模型或稍后再试」）。换不动（预算尽 / 没人接 / 唯一候选是 mock）交回 `backoffRequeue` 原退避路径；一家都接不下时提交侧仍是 503 `no_provider_available`（「所有供应商暂时繁忙，请稍后再试」）。**读超时 / 中途断连 / 裸 5xx 永远到不了这里**——`phase:"read"` 与无 phase 的网络错都按「可能已受理」走 `uncertain_submit`。轮询成败只记 `recordOutcome`（失败计入健康、可能触发冷却），任务按原路径失败退款，**绝不重发**。
...
`POST /api/jobs` 的 `model` 字段(`createJobBodySchema`,可选,≤64 字符)传的是产品 id。`resolveProductChoice` 是 create/retry/智能体报价共用的解析入口：指定产品时解出 provider 与上游模型并按产品能力做 400 校验；未指定时沿用 §2b/§2c/§2e 的 ORDER + 能力路由，再按 provider 与上游模型反查实际产品。`JobRecord`/`JobPublic` 写入 `product`(id)/`productName`，前端与详情卡显示 `productName`。
...
`priceCny(input)` 按 mode 取值:视频 ≤5s/更长两档基价,1080p 乘 `hd`,有声再加 `audio`;30/45/60 秒 harness 长片走 `longForm` 定档(默认 20/30/40 元,`hd` 倍率与 `audio` 加价照常叠加——长片含角色表/首帧生图与 Director 成本,段数×基价兜不住);`extend_video`/`edit_video` 是定值;`text_to_image` 按 `imageResolution` 取 `1k`/`2k` 档。`LUMEN_PRICE_TABLE`(JSON,可只写要改的几项)覆盖默认表,坏 JSON 记一条 warn 并回落默认,不挡提交。产品可带 `ProductPriceOverride {video?,image?}`；`priceTableFor(base, product.price)` 只逐字段覆盖 `video`/`image` 的同名售价，非法、负数或非有限数字逐字段忽略，未传 override 时原样返回 base 引用。`createJob`/`retryJob` 按**归一后**的参数和本次实际落点产品算 `priceCny` 并写入 `job.priceCny`；源产品不可用而重试改落别家时不沿用源产品 override。提交后售价永不改写——它同时是在途预留额和成功后扣款额。
...
- API:`GET/POST /api/agent/sessions`、`GET/PATCH/DELETE /api/agent/sessions/:id`(PATCH 收 `title`/`budgetCny`)、`POST /api/agent/sessions/:id/messages`(20 次/分钟/用户)、`GET /api/agent/sessions/:id/turns/:turnId`、`POST .../turns/:turnId/approve|reject`、`GET /api/agent/skills`(下发 `kinds` 与对话模型 `chat`)。
- 前端 `src/components/genius/agent/**` 全接真数据：首页与对话页共用 `AgentPickers` 的模型/图片/视频/技能四枚芯片；模型弹层把真实模型列表与只调温度/篇幅的创意档分组。待批提案显示产品名、报价、合计与批准/拒绝，助手落款显示实际模型与创意档；批准后照常轮询任务。预算芯片可设/解除；不可用时置灰「智能体暂未开放」。
...
- **前端** `CanvasView.tsx`:右键菜单加四类节点、拖拽定位、文本/提示词防抖 600ms 落盘、素材上传走 `POST /api/uploads`、生成节点轮询 `jobId` 恢复产物;PATCH 409 `revision_conflict` 时保留本地 doc 不动、弹 `.canvas-conflict` 弹层列「本地(未保存)/ 服务端」两份摘要(节点/连线数 + 服务端 `updatedAt`)让用户二选一——「保留本地并覆盖服务端」按 `conflict.server.revision` 重发 PATCH(再 409 就刷新服务端栏继续),「采用服务端」丢弃本地;严格模态——只能点这两个按钮关,Esc / 点外层无动作(避免误触替用户选边)。冲突未决期间 `persist` 不再发 PATCH。窄屏(≤560px)不再按 fit 缩小场景层——375 下 fit 会算到 0.23、节点缩成指甲盖,这一档固定 1:1 并靠 `.canvas-scroll` 滚动平移,左侧不给未渲染的工具箱留位,运行钮铺成整行;建节点除右键外支持长按 500ms(开菜单后 400ms 内不接受点击,挡掉抬手补发的那一下),节点拖拽改 `pointerdown` 并监听 `pointercancel`、拖拽手柄 `touch-action:none`,无 hover 的设备上删除钮常显、几个最小控件加大热区(review 2026-09-15 U-02 / C-22)。
...
- **能力与目录字段**：`modes` = 配了默认模型的视频 mode + 有 image 通道时的 `text_to_image`；画幅 / 时长档 / 分辨率 / 参考图上限全部从 `catalog.models` 推导。每个模型除 aliases/durations/resolutions/ratios/credits 外可配置 `name`（用户展示名）、`hidden`、`kind:video|image|chat` 与 `price`；展示名顺序为配置 `name` → 快照 `upstreamName` → id。积分成本按 `credits = 分辨率价 + 时长价`，`creditsPerCny` 折人民币再折 USD。
- **动态目录（N3.3）**：`catalog.source:"models-endpoint"` 时目录 = `/models` 快照（`data/relay-catalog/<id>.json`）∪ 配置覆盖；同一模型经 `mergeModelSpecs` 按字段深合并，配置里的 `undefined` 不覆盖快照，`credits`/`price.video`/`price.image` 也逐层合并，配置只写 hidden 或 price 不会丢目录能力。启动 30s 后首拉，之后每 `RELAY_CATALOG_REFRESH_MS`（默认 30 分钟、下限 60 秒）刷新，失败保留上次快照；discover 可手动刷。快照一律原子写（临时文件 + rename），读侧那行缓存的键是 `mtimeNs:size:ino` 且只在「上次读发生在文件最后一次修改之后」时才生效——只按 `mtimeMs` 记的话，同一毫秒内的两次写会让读者一直拿到上一版目录（review 2026-09-15 B-03）。每次刷新做 diff；默认模型消失时对应 mode 从 capabilities 收缩，回来自动恢复。
- **产品生成（N3.3）**：`allProducts()` = 手写内置产品 ∪ 显式 file/env-seed relay 的可售目录模型，产品 id 为 `<relayId>:<slug>`（slug 小写、非 `[a-z0-9]` 转 `-`、28 字符截断、重复加 `-2`）。video 至少配置 `price.video["5"]` 与 `["10"]`，image 至少配置 `price.image["1k"]` 才上架；`hidden:true`、`kind:"chat"`、未定价或已被同 provider 默认产品钉住的模型不生成，避免重复和按全局默认价误售。image 生成 `text_to_image` 产品，video 的档位/能力来自目录；`LUMEN_PRODUCTS` 仍可按 id 覆盖。`GET /api/models` 下发 `price` override、providerId/providerName/upstreamModel/costHint；面板与服务端都以 `priceTableFor(base, product.price)` 计算售价。`isProductAvailable` 还要求模型仍在实时目录与 provider ORDER 内。
...
- **管理接口** `src/app/api/admin/relays/`（登录 + `LUMEN_ADMIN_USER_ID`，非管理员一律 404）：`GET /` 返回 relay、health 与合并目录模型（含来源、listed 状态，不回显 key）；`POST /`；`PATCH /:id`；`DELETE /:id`；`POST /:id/discover|probe`。PATCH 的 `catalog.models` 是按模型 key 的部分更新：给出的模型 spec 整体替换该 key 的配置覆盖，未给的模型保持，值为 `null` 删除该覆盖；id 不可改。写操作原子落 `relays.json` 后立即 reconcile。env 预设不由 PATCH/DELETE 管理（404），须先 POST 同 id 文件条目覆盖；probe 可能在上游计费且付费 POST 固定 `maxAttempts:1`。
- **管理页 `/admin/relays`（N3.5，2026-09-13）**：`(shell)` 路由，服务端 `isAdminUser` 判定、非管理员 `notFound()`；入口只随 `caps.isAdmin` 露出。列表按 priority 升序显示来源、keyEnv+hasKey、通道、health、目录来源/快照；`.relay-models[data-relay-id]` 折叠模型表逐行编辑 kind、展示名、hidden、视频/图片售价、参考图数与时长档，并显示已上架/未定价/已隐藏/默认表已含等状态，保存只 PATCH 改过的模型。legacy 行只读，提供「转为可管理条目」。启停/排序/删除只作用于文件条目，排序用上移/下移按钮换相邻 priority；probe 前确认可能计费。
...
- **重试涨价先确认**(B-10):重试按**当下**的参数与落点产品重新计价(可灵时长归一、换家、价表或产品 override 变动都会让它变)。新价高于源任务且 `source.priceCny > 0` 时,`retryJob` 在 `reserveJobFunds` **之前**抛 409 `retry_price_changed`,并经 `ProviderHttpError.publicFields` 回传 `{priceCny, previousPriceCny}`;调用方带 `acceptPriceCny === 新价` 再来一次才放行(值对不上照样拦,不拿旧报价换新价)。降价不拦。`source.priceCny > 0` 是必须的:计费模型之前落盘的老记录读回来都是 0,不加这条会把那批存量任务的每次重试都拦住。界面上按钮变成「确认重试（¥x）」,换一条任务即清掉确认态。
- boot recover(`instrumentation.register` → `startJobRunner`,幂等,**2026-09-06 阶段一改写单片分支**;2026-09-19 起**只读非终态索引再按 id 回读**,不再 `listJobRecords()` 全量读 job.json——启动时 `ensureJobIndex()` 已按目录读过一遍,recover 关心的只有还没到终态的那几条,作品累积之后全表扫会让每次重启/部署的白屏窗口线性变长,B-11):`submitting` 且**无** remoteId 不再无条件回 queued——先调 provider 可选的 `lookupByExternalId(jobId)`(可灵已实现,按 `external_task_id` 查)问上游是否已经接过这个请求;查到就把返回的 remoteId 写回 job.json 转 `pending` 续跑,查不到(或 provider 未实现该方法、或查询本身失败)就转 `failed` + `error.code="uncertain_submit"`,由 `retry-guard` 的 `retryBlock()` 拦一键重试(与 harness 分镜级的同名标记共用一套拒绝逻辑与文案模板,§7.2)。`submitting` 有 remoteId → 改 pending 续跑;`pending/persisting` 续跑;超 15min 的 **submitting/pending/persisting/harness 各阶段** 标 expired(这条晚于「uncertain」判定执行,陈旧与「上游是否已接单」是两个互不隶属的问题);`queued` 一律重新入队,不因排队久而失败。harness 阶段的任务由 pump 重新交给 `orchestrator.execute`,它按 job.json 里的 plan / shot 记录续跑(shot 级 recover 见 §7.2)。
...
- **响应体的静默上限(2026-09-17,`providers/grok/client.ts` `watchBodyIdle`)**:`UPSTREAM_TIMEOUT_MS` 的 abort 定时器在**响应头**到达时就被清掉,此后读响应体没有任何时限——上游发完头不再发数据也不断开时,`res.json()` 与成片下载的 `pipeline()` 都会永远挂着,任务于是永远停在 `pending` / `persisting` 这种非终态上:界面上的画布节点与创作页任务卡一直转圈,刷新也没用(服务端记录本来就没到终态),`pollUntilDone` 的 15 分钟总时限也轮不到执行(它在 `while` 条件上,而代码停在 `await` 里)。现在 `fetchUpstream` 把 abort 控制器接到响应体上:每块数据续一次看门狗,`UPSTREAM_BODY_IDLE_MS`(默认 60s,上限 10min)内一块都不来就 abort。判据是**静默**而非总时长,慢但持续传输的大文件不受影响。失败按普通读错误上抛,由 runner 落 `failed` 并退款——用户看到的是一条失败与「重新生成」,不是一个永远转的圈。所有 provider(relay/YMan、可灵、Grok、openai-image)共用这一条传输层,下载与轮询同受保护。浏览器侧同理给轮询请求(`fetchJob`/`fetchCanvasRun`)加了 `AbortSignal.timeout`:任务页的轮询是「上一拍回来才排下一拍」的链,一次永不 settle 的请求会把整条链静悄悄掐断。
...
| 端点 | 说明 |
| --- | --- |
| `POST /api/uploads` | multipart 流式(@fastify/busboy);`role ∈ start|last|reference|source_video`;图 ≤**6MB**(2026-09-06 阶段一从 12MB 下调,sharp 后覆盖写)、视频 mp4 ≤**24MB**(从 48MB 下调,ffmpeg 探针,产线 2 核/1.8G/`MemoryMax=700M` 下的内存预算,见 §3.3 与 `docs/plan-architecture-2026-09.md` P1);写 `data/tmp/{up_16hex}` + sidecar json;**不**做模式相关校验、不调 Files。解不开的输入(选了 `.txt`、改过扩展名的假图、半截文件、空字节)在 `media/preprocess.ts` 统一包成 **400 `invalid_argument`** 中文文案,不再让 sharp 的英文原文经 `jsonError` 的 500 兜底直达浏览器;体积上限的数值在 `media/upload-limits.ts`,浏览器 import 同一份,在本地先按 `file.type`/`file.size` 拦一次并把原因写进 `.composer__error`(review 2026-09-15 U-01) |
...
| `POST /api/jobs/:id/cancel|retry` | 见 §3;retry 同样受 402 余额判定,涨价时先 409 `retry_price_changed`(body 带 `priceCny`/`previousPriceCny`),带 `acceptPriceCny` 重提才放行 |
...
- **供应商无关（2026-09-13 E1+E2 + 验收修复落地，`02163a0`/`bb22d75`/`545580f`，已部署且生产已开）**：shot 路由枚举 `t2v / i2v / r2v`（`ShotRoute`），续接一律「上一镜尾帧 → 下一镜 i2v」（`tail_chain`），不再有 extend 片段，也不碰任何一家的私有 Files API。长片任务在 `selectProvider`/`currentProviderId` 里按 `image_to_video` 模式走 `pickVideoProvider` 的正常 ORDER（要求该 provider 同时声明 `text_to_video`，`requireModes` 约束），可灵（t2v/i2v、5/10 秒档）与 YMan（t2v/i2v/r2v）都能承接；`assertProductFit` 对长片同样要求产品 provider 声明 t2v+i2v。`harnessSettingsFor`（`src/lib/jobs/provider-settings.ts`）以合法单段时长 10 秒调用 `providerSettingsFor` 取归一后的 resolution/audio/ratio，`durationSec` 仍取目标总长 30/45/60——创建、重试、确定拒单换家（`switchProvider`）三处共用。30/45/60 永不进任何原生请求体——grok 的 rest-map 仍对这三个时长拒 `harness_duration`（golden 保障）。生产 `HARNESS_ENABLED=true`，可灵 30s 长片已实证（`docs/acceptance-2026-09-13.md`：3 镜 10s、档A 三视图 + 首帧生效、30.97s 成片）。
- **分镜级换家（2026-09-13 N3.4，as-built）**：单镜 `submit` 被上游**确定拒绝**（`isCertainRejection`，口径同 §2e）时只换这一面镜子——`run-persisted-shot.ts` 的默认 `onCertainRejection` 回调把被拒的家记进 `HarnessShotRecord.excludedProviders`（随记录落盘，跨重启的重试也不再投给它），用 `currentProviderId("image_to_video", {harness:true, exclude})` 重选落点、按新家各 mode 重取模型（产品标签只在本家内生效，换家后不沿用旧家产品查模型），换家上限 `RELAY_MAX_SWITCHES` 按镜计；落点不支持 r2v 时经 `shot-router.ts` 的 `downgradeR2vShot` 按 lockPlan 同规则降 i2v/t2v（有首帧或续接镜 → i2v，否则 t2v）。record 写回 `provider`/`model`/`excludedProviders`，估价改按 `record.provider`/`record.model`（`shotPricingFor`）；模糊失败（读超时 / 断连 / 裸 5xx）不调回调，直接 `uncertain_submit` → needs_review。已 `succeeded` 的镜子在 `executeShotWithRetries` 入口原样返回，别家换家动不到它。
- `Harness Director`：`src/lib/harness/director.ts` 走 `agentLlmConfig()`（`AGENT_API_KEY` → `AGENT_BASE_URL`，生产 ccgoai `gpt-5.6-luna`；其下回落 XAI；都没有且非 mock → `HarnessFailure("llm_unavailable")`），`response_format` 用 `json_object`，严格 JSON Schema 以文本嵌进 system prompt、Zod 本地校验，格式失败最多重试 2 次。Director 与视觉 QC 的 LLM 调用走独立超时 `HARNESS_LLM_TIMEOUT_MS`（默认 120s、上限 5min——完整计划实测常超通用 `UPSTREAM_TIMEOUT_MS` 的 30s）；上游超时/错误归一成 `HarnessFailure("llm_upstream_failed")`（不产生付费分镜、不锁重试），不再是裸的 "Request timed out."。mock 模式（`agentLlmConfig().provider === "mock"`）用 `mock-director.ts` 的确定性计划：首镜 t2v、其后 tail-chain i2v，全部 10 秒段。
- **开关（as-built）**：`HARNESS_ENABLED` 未开启时 `orchestrator.execute` 抛 `HARNESS_NOT_ENABLED`、API 对 30/45/60 返回 400；开启后 `createJob` 接受 30/45/60（仅 t2v / i2v），`costUsdEstimate` 由 `harnessSubmitEstimateUsd`（`provider-settings.ts`）预估 = 视频片段按 `packHarnessDuration` + 选中 provider 的 `estimateCostUsd` + `LLM_RESERVE_USD.director` + `HARNESS_IMAGE_SHOT_COUNT`（4 = 单角色三视图 + 一镜首帧的经验值）× 当前 IMAGE ORDER 首选 provider 的 16:9/1k 生图估价，多角色/多 hard_cut 镜会超出、`costOverTarget` 软线与 ×2 硬上限照常告警；Director 出计划后按真实 packing 重算为 `costUsdPlanned`。`/api/health.harnessRunnable` 反映开关。
...
- 独立止损阀 `FREE_DAILY_FAILURE_LIMIT`(默认 30):账号每日失败/取消次数超限即拒绝新提交,防止有人靠反复失败消耗上游余额;它的优先级高于配额判定。文案写明「失败与取消」和「北京时间 0 点重置」(B-06):它本来就是日窗口自动重置的,原来那句「请联系管理员」既不成立也没有任何用。
...
`src/lib/jobs/retention.ts`:终态任务且 `completedAt ?? updatedAt` 超过 `DATA_RETENTION_DAYS`(默认 30,0 关闭)时删 `outputs/`、`inputs/`、`shots/` 三个目录,写 `artifactsPurgedAt`(同一次 `updateJob` 内,先给缺 `completedAt` 的老记录补章再写清理时间戳,保证幂等)——**只写这个字段,不改 `status`**,不碰任何非终态任务。runner 每小时的 `maintenance()` 依次跑 tmp 清理 → idempotency 24h 清理 → retention。已清理任务禁止一键重试(`retryJob` 返回 409 `artifacts_purged`,输入已删,UI 引导「用这条提示词重新生成」走全新提交与全新配额);UI 画廊对 `artifactsPurgedAt` 非空的任务显示占位卡「作品已过期清理」,不请求已删除的 media。清理是**静默**的,所以 `toPublic` 另派生一个只读字段 `artifactsExpireAt`(= `completedAt ?? updatedAt` + `DATA_RETENTION_DAYS`,与 `shouldPurgeArtifacts` 共用 `jobs/retention-window.ts` 的同一段算术;留存关闭 / 非终态 / 已清时为 null),**不落盘**——`DATA_RETENTION_DAYS` 随时可改,存下来的值第二天就可能是错的。前端据此在剩余 ≤7 天时给作品卡加 `.masonry__expiring` 角标,详情浮层常驻一行 `.work__expire` 说明保留到哪天、还剩几天、到期前请下载(review 2026-09-15 B-05)。倒计时只在挂载后算,首屏 SSR 不渲染它(服务端与浏览器的「现在」不是同一刻)。取消操作触发的 `job failed` 日志级别由 warn 降为 info,避免和真实失败一起淹没日志。
