...
- **动态目录（N3.3）**：`catalog.source:"models-endpoint"` 时目录 = `/models` 快照（`data/relay-catalog/<id>.json`）∪ 配置覆盖；同一模型经 `mergeModelSpecs` 按字段深合并，配置里的 `undefined` 不覆盖快照，`credits`/`price.video`/`price.image` 也逐层合并，配置只写 hidden 或 price 不会丢目录能力。启动 30s 后首拉，之后每 `RELAY_CATALOG_REFRESH_MS`（默认 30 分钟、下限 60 秒）刷新，失败保留上次快照；discover 可手动刷。每次刷新做 diff；默认模型消失时对应 mode 从 capabilities 收缩，回来自动恢复。
