...
- 登录版已上线（部署 `4ad5d60`），留存清理版（`9211324`）正在准备部署。`/` 307→`/login`，`/api/health` 匿名 200，其余 `/api/*` 401。
...
- `main`(`4ad5d60`)**落后** `integrate/users2`（缺第五批留存清理与 shots 清理）：主树有用户未提交的 ThreeUI 丝绸幕布实验（`ClothVeil.tsx`、`src/shaders/`、`globals.css`、`LumenHome.tsx`、`next.config.ts` raw-loader、`handoff.md` §0a），与登录页改动同文件，待用户实验告一段落后三方合并。
