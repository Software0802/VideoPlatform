...
| 门禁 | 本轮依次执行 `pnpm exec tsc --noEmit` → `pnpm exec eslint src` → `pnpm test` → `pnpm build` → `pnpm e2e`，均退出 0：全量 96 文件、1131 通过、1 skip（`run-graph.test.ts` 24 条：切片一的环/容量/幂等/`quote_stale`/链式接续/取消/崩溃接管/素材不消耗/`price_changed`，切片二的预算冻结与终态释放、份额转移恰好计一次、孤儿 transfer 兜底计占用、坏文件 `billing_state_corrupt` 失败关闭、复用 ¥0 采纳、显式重跑、`output_purged` 阻断、`expandRegenerate` 闭包、审批门批准/驳回）；e2e 29 通过 |
