...
## 画布 DAG 经 UI 走真实上游（部署 `9e0441c` 后，Playwright 操作公网站点）

画布 `cv_462853265df0`，run `crun_85ef040816c3`：右键建「文生图」→ 右键建「生成视频」并在下拉里选上游为该图节点 → 「运行整图」报价 ¥0.5 + ¥4（i2v 10s）= ¥4.5 → 确认后冻结 → 文生图 `job_a2175de52913` 完成 → 视频节点进入「待批准」（显示 09-14 12:44 前批准否则跳过，即 24h 审批超时）→ 批准 → 图生视频 `job_872e8536bf28`（kling，以上游图为首帧）完成。流水扣 0.5 + 4，余额 5.45 → 0.95，与报价一致；两节点缩略图 / 视频在画布上可见。
