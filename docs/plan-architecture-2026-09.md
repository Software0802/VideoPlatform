...
| 字段 | 值 |
| --- | --- |
| 状态 | 阶段一 / 阶段二已实施并部署生产，as-built 见 `docs/design.md` §2d/§2g/§3/§12 与 `docs/handoff.md`；阶段三（多实例 / 数据库）未启动。本文正文为历史路线图，不再维护。2026-09-13 起产品方向为多中转按能力路由，Grok 为普通可选成员；本文 Grok 主视角部分为历史决策，现状以 `docs/design.md` 为准 |
...
| 余额 | 新用户初始 0；管理员 CLI `grant-balance` 手动充值；人民币定价（默认价目：5s 视频 ¥2、10s ¥4、1080p ×1.5、有声 +1、延长 ¥3、编辑 ¥4、图片 1K ¥0.5 / 2K ¥1，`LUMEN_PRICE_TABLE` 可覆盖）；支付网关第三阶段 |
